...
if(!file.exists("household_power_consumption.zip")){ 
    ...
    unzip(zipfile = "household_power_consumption.zip", files = "household_power_consumption.txt", overwrite = T)
}

## Load data into R and convert dates to Date format
powerConsumption <- fread(file = "household_power_consumption.txt", na.strings = "?")
